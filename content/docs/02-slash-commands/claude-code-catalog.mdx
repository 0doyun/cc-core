...
- Claude 의 Bash 호출 전에 린트를 강제하거나, Edit 후에 포맷터를 돌리거나 하는 식의 "자동 거드름" 을 박는 층.
...
- `low` / `medium` / `high` 는 세션 간 유지되고, `max` 는 현재 세션만·Opus 4.6 전용입니다.
- `auto` 로 기본값 복귀. 복잡한 알고리즘 문제는 `max`, 단순 치환 작업은 `low` 가 비용 대비 효율이 좋아요.
...
<Callout type="info">
**Last verified: 2026-04-11** — Claude Code v2.1.x 기준. 커맨드 추가·제거는 빠른 편이라 구글 검색으로 오래된 가이드를 주울 때 특히 주의하세요. 공식 Release Notes 가 가장 확실한 1차 출처입니다.
</Callout>
