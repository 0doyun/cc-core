...
- `low` / `medium` / `high` 세 단계. 세션 간 유지됩니다. (과거 존재하던 `max` 는 v2.1.72 에서 제거.)
- `auto` 로 기본값 복귀. v2.1.94 부터 유료 사용자(API·Team·Enterprise) 의 기본값이 `high` 로 변경됐어요. 단순 치환 작업은 `low` 로 내려서 비용을 아낄 수 있습니다.
...
<Callout type="info">
**Last verified: 2026-04-12** — Claude Code v2.1.101 기준. 커맨드 추가·제거는 빠른 편이라 구글 검색으로 오래된 가이드를 주울 때 특히 주의하세요. 공식 Release Notes 가 가장 확실한 1차 출처입니다.
</Callout>
