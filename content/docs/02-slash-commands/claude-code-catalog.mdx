...
- `/claude-api` — [Skill 기반] Claude API / Anthropic SDK 앱 빌드·디버그·최적화. 프롬프트 캐싱, tool use, batch 등 API 기능 작업 시 자동 트리거.
- `/statusline` — [Skill 기반] 상태 표시줄 UI 설정. 레이아웃·표시 요소를 커스터마이즈.
- `/team-onboarding` — [Skill 기반] 내 사용 패턴을 분석해서 팀원용 Claude Code 온보딩 가이드를 생성.
...
[공식 Release Notes](https://code.claude.com/docs/en/release-notes) 에서 버전별 제거·추가 내역을 추적할 수 있어요.
