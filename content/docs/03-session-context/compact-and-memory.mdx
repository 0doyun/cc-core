...
[공식 Context Window 문서](https://code.claude.com/docs/en/context-window) 및 커뮤니티 분석([Decode Claude](https://decodeclaude.com/compaction-deep-dive/)) 종합.
...
### 2-1. 4 개 공식 스코프 + 2 개 보조 메커니즘

[공식 Memory 문서](https://code.claude.com/docs/en/memory) 는 4 개 스코프(관리 정책·프로젝트·개인 전역·개인 로컬) 를 정의합니다. 여기에 서브디렉토리 지연 로드와 `rules/` 를 포함하면:
...
상대 경로는 import 하는 파일 기준이고, 재귀는 최대 5 단계입니다.
...
`/context` 는 색상 그리드로 현재 컨텍스트 사용률과 각 카테고리 비중을 보여줍니다. 말 한 번 하기도 전에 시스템 프롬프트 + CLAUDE.md + auto memory 등으로 **이미 수천 토큰이 먹혀 있어요.** `/context` 로 확인해서 프로젝트 `CLAUDE.md` 가 1,500 토큰을 넘으면 `@import` 구조를 가지치기하세요. 각 항목별 정확한 토큰 수는 [공식 Context Window 시뮬레이터](https://code.claude.com/docs/en/context-window) 에서 확인할 수 있습니다.
...
<Callout type="warn" title="60% 룰 — 컨텍스트 60% 시점에서 실행">
...
<Callout type="warn" title="컴팩션 후 드롭 검증">
...
요약이 엇나갔다면 이 시점에 교정 가능합니다. 이 확인 없이 그냥 작업을 이어가면 **드롭된 컨텍스트를 한참 뒤에 발견**하게 돼요. 출처: 커뮤니티 실전 경험칙.
</Callout>

<Callout type="warn" title="Auto memory 비대화 방지">
...
<Callout type="warn" title="@import 재귀 깊이 주의">
...
<Callout type="info">
**Last verified: 2026-04-15** — Claude Code v2.1.109 기준. Auto memory 는 v2.1.59 이상에서만 동작. 공식 문서 도메인은 `code.claude.com/docs/en/*` 로 이전됐습니다(`docs.anthropic.com/en/docs/claude-code/*` 는 301 리다이렉트).
</Callout>
