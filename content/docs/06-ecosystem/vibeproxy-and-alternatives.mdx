...
[하네스 엔지니어링](/docs/00-start/harness-engineering) 에서 설명한 것처럼, Claude Code 는 **runtime harness** 입니다. 모델은 교체 가능한 부품이에요. `ANTHROPIC_BASE_URL` 환경변수 하나면 Claude Code 가 바라보는 백엔드를 통째로 바꿀 수 있습니다.

```mermaid
flowchart LR
    CC["Claude Code<br/>(하네스 그대로)"]
    ENV["ANTHROPIC_BASE_URL<br/>환경변수 1개"]
    A["Anthropic<br/>(기본)"]
...
이게 전부. 이제 Claude Code 의 모든 기능 — `/compact`, `/plan`, Hooks, subagent 위임 — 이 GPT 백엔드에서 돌아갑니다. CLAUDE.md 에 적어둔 규칙도 그대로 적용돼요.
...
<Callout type="info">
**Last verified: 2026-04-12** — vibeProxy v1.8.135 기준. 프록시 생태계는 변동이 빠르니 각 레포의 최근 릴리즈를 확인하세요.
</Callout>
